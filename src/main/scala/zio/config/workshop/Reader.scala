...
/**
 * zio-config aims to be one-stop-shop for all
 * configuration management usecases in Scala.
 *
 * It can do the following
 * 1. It can read config from various sources such as system env, system properties, hocon/json (yaml PR in progress).
 ...
 * 4. Manually writing the config is at its first-class support. Meaning the design is not too tightly coupled with typeclass derivation and implicits.
...
object Reader extends EitherSupport {
  ...
  object Credentials {
    val defaultConfig: ConfigDescriptor[Credentials] =
      (string("username") |@| string("password"))(Credentials.apply, Credentials.unapply)
  }

  import Credentials._

  /**
   * Read the given map to Credentials using simple map.get.
   * Make sure errors are accumulated if there are no
   * @return
   */
  ...
  /**
   * Read the given map to Credentials using defaultConfig description
   * Make sure errors are accumulated if there are no
   * @return
   */
  def readSimpleMap: Either[ReadError[String], Credentials] = {
    ...
    read(defaultConfig from source)
  }

  /**
   * Read the given map to Credentials
   * but make sure the errors are accumulated since there are no
   * keys that match the ones in description
   */
  /**
   * Read the given map to Credentials.
   * Make sure errors are accumulated if there are no
  ...
  def readSimpleMapWithError: Either[ReadError[String], Credentials] = {
    ...
    val config: ConfigDescriptor[Credentials] =
      (string("USERNAME1") |@| string("USERNAME2"))(Credentials.apply, Credentials.unapply)
    ...
    // Calling this function should return the following error
    // Discuss more on error messages using Either
    /*
    Left(ReadError:
        ╥
        ╠══╦══╗
   ...
   * Instead of using Config.fromSystemEnv, use the ConfigSource and read functions
   */
  def readCredentialsFromSystemEnv: IO[ReadError[String], Credentials] = {
    for {
      source <- ConfigSource.fromSystemEnv
      config <- ZIO.fromEither(read(defaultConfig from source))
    } yield config
  }

  /**
   * Read the config from System properties to Credentials case class.
   * Instead of using Config.fromSystemProperties, use the ConfigSource from read functions
   */
  def readCredentialsFromSystemProperties = {
    for {
      source <- ConfigSource.fromSystemProperties
      config <- ZIO.fromEither(read(defaultConfig from source))
    } yield config
  }
   ...
   */
  def readCredentialsFromSystemHocon: Either[String, Credentials] = {
    val hocon =
      s"""
         |
         | {
